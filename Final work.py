"""
=============================================================================
ThreatScan Pro — Final Merged Build
=============================================================================
Frontend  : gemini-code-1777944143626 (Professional dark dashboard UI)
Backend   : ThreatScan Pro v5.5 (Weighted RF + SQLite forensic logging)
Dataset   : 5,000 URLs — 2,500 Benign + 2,500 Malicious (augmented)
Models    : Random Forest · Linear SVM (calibrated) · Decision Tree
Features  : 36 lexical + host-based URL features
Security  : html.escape (XSS) + Parameterized SQL (SQLi)
Stability : st.cache_resource + session-state persistent results
=============================================================================
FIXES APPLIED (5-pass cross-check):
  [1] Dataset size corrected: create_dataset(2500, 2500)
  [2] log_prediction key unified: 'processing_time_ms' → 'processing_time'
  [3] augment_urls signature unified (frontend style, backend seed lists)
  [4] FEATURE_COLUMNS derived from extract_features() — consistent across train/predict
  [5] st.set_page_config() is absolute first Streamlit call (before init_database)
=============================================================================
"""
...
def _ensure(pip_name: str, import_name: str = None):
    ...
    try:
        ...
        print(f"Attempting to auto-install missing package: {pip_name}...")
        try:
            try:
                # Try uv pip install first since it's a uv environment
                subprocess.check_call(["uv", "pip", "install", pip_name])
            except (subprocess.CalledProcessError, FileNotFoundError):
                # Fallback to standard pip; --break-system-packages required on newer Ubuntu
                try:
            ...
            print(f"\n[!] Failed to auto-install '{pip_name}'. Please run 'uv pip install {pip_name}' manually.\n")
            sys.exit(1)
...
# --- Expanded seed pools (merged from both codebases) ---
BENIGN_SEEDS = [
    ...
    "https://www.dropbox.com/s/example/file.pdf",
...
MALICIOUS_SEEDS = [
    ...
    "http://0x58.0x23.0x11.0x01/login?session=abc",
    "http://chase-online-update.xyz/auth?token=xyz",
    "http://xn--gogle-pua.com/verify?id=12345",
    "http://dhl-parcel-tracking.site/track?id=99999",
    "http://wellsfargo-secure-auth.ga/signin",
    "http://secure-login.paypa1.com.verify-account.tk/update",
    "http://login.microsoftonline.update.ml/oauth2/authorize",
    "http://amazon-winner-2026.top/claim?ref=email",
    "http://steam-gift-card-free.top/redeem?code=WIN",
    "http://whatsapp-account-suspended.live/reactivate?user=abc",
    "https://govt-police.club/nz",
    "http://gov-tax-refund-alert.info/claim",
]


def augment_urls(url_list: list, target_count: int) -> list:
    """Augment a seed list to target_count by appending randomized query params."""
    # FIX 1: Deduplicate seed list to prevent skewed random selection probabilities
    ...
    while len(augmented) < target_count and attempts < max_attempts:
        ...
        # FIX 2: Prevent malformed trailing separators like "?&" or "??"
        if base.endswith("?") or base.endswith("&"):
            sep = ""
        else:
            sep = "&" if "?" in base else "?"
            
        key = random.choice(["ref", "id", "session", "token", "user", "auth", "track"])
        candidate = f"{base}{sep}{key}_{random.randint(1, 999)}={random.randint(1000, 999999)}"
...
def create_dataset(n_benign: int = 2500, n_malicious: int = 2500):
    """Generate a balanced 5,000-URL dataset and write to CSV."""
    rows = (
        [(u, 0) for u in augment_urls(BENIGN_SEEDS,   n_benign)]
      + [(u, 1) for u in augment_urls(MALICIOUS_SEEDS, n_malicious)]
    )
    random.shuffle(rows)
...
def ensure_dataset():
    """Ensure dataset exists and is perfectly valid."""
    needs_creation = True
    if os.path.exists(DATASET_PATH):
        try:
            # FIX 3: Cross-check that the file isn't empty and has exactly the correct row count
            with open(DATASET_PATH, "r", encoding="utf-8") as f:
                row_count = sum(1 for _ in f)
            if row_count >= 5001:  # 5000 rows + 1 header
                needs_creation = False
        except Exception:
            pass
            
    if needs_creation:
        create_dataset(2500, 2500)  # FIX [1]: 2500 each


# =============================================================================
# SECTION 3 — FEATURE EXTRACTION (36-POINT DNA)
# =============================================================================
SUSPICIOUS_TLDS  = {'xyz','top','click','tk','ml','ga','cf','gq','pw','cc','su','biz','info','online','site','live','stream','club','vip','shop','store'}
TRUSTED_DOMAINS  = {'google.com','youtube.com','facebook.com','microsoft.com','apple.com','amazon.com',
                    'github.com','twitter.com','linkedin.com','wikipedia.org','paypal.com','netflix.com'}
BRAND_KEYWORDS   = ['paypal','google','apple','microsoft','amazon','facebook','instagram','netflix',
                    'ebay','steam','whatsapp','youtube','chase','wellsfargo','dropbox','govt','police','gov','irs','tax']
...
SPAM_WORDS = ['free','win','prize','claim','urgent','alert','suspended','verify','confirm','limited','offer','lucky','bonus']
...
def extract_features(url: str) -> dict:
    ...
    f['domain_age_days']    = 365  # placeholder — constant; does not affect relative model ranking
    return f


# FIX [4]: FEATURE_COLUMNS derived once from extract_features for consistent train/predict alignment
FEATURE_COLUMNS = list(extract_features("http://example.com").keys())


# =============================================================================
# SECTION 4 — DATABASE (SQLite + Auto-Migration)
# =============================================================================
def init_database():
    ...
    ''')
    # Safe migration for existing databases missing actual_label column
    try:
...
def log_prediction(data: dict):
    # FIX [2]: unified key name — 'processing_time' (not 'processing_time_ms')
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute(
        ...
        (
            ...
            data.get('safe_pct', 0),
            data.get('mal_pct', 0),
            data.get('processing_time', 0),     # key matches predict_url output
        )
    )
    scan_id = c.lastrowid
...
def update_actual_label(scan_id: int, label: int):
    conn = sqlite3.connect(DB_PATH)
    conn.execute('UPDATE scans SET actual_label = ? WHERE id = ?', (label, scan_id))
    conn.commit()
    conn.close()
...
# =============================================================================
# SECTION 5 — MODEL TRAINING (3 classifiers, best auto-selected)
# =============================================================================
def train_all_models():
    ensure_dataset()
    
    # FIX 4: Bulletproof the read operation so a completely broken CSV forces a rebuild
    try:
        df = pd.read_csv(DATASET_PATH)
        if df.empty or 'url' not in df.columns:
            raise ValueError("Corrupted CSV")
    except Exception:
        create_dataset(2500, 2500)
        df = pd.read_csv(DATASET_PATH)
        
    df = df.drop_duplicates(subset=['url']).dropna(subset=['url', 'label'])
    df['label'] = df['label'].astype(int).clip(0, 1)

    # FIX [4]: use FEATURE_COLUMNS from module scope — same derivation as predict_url
    X = pd.DataFrame([extract_features(u) for u in df['url']])[FEATURE_COLUMNS].fillna(0).values.astype(float)
    y = df['label'].values

    # Impute placeholder domain_age_days column
    ...
    classifiers = {
        "Random Forest": RandomForestClassifier(
            ...
            class_weight={0: 1, 1: 3},   # v5.5: aggressive phishing recall weighting
    ...
    for name, clf in classifiers.items():
        ...
        # Guard: always use labels=[0,1] so matrix is always 2×2
        ...
        # Guard: roc_auc_score fails when only one class present in y_test
        try:
    ...
    with open(COMPARISON_PATH, 'w') as fh:
        json.dump(
            {n: {k: v for k, v in d.items() if k not in ('y_test', 'y_pred', 'y_prob')}
...
def load_model():
    ...
    try:
        ...
        # Corrupted or incompatible model file — delete and retrain
        try:
...
def predict_url(url: str, mdl, feat_cols: list) -> dict:
    start  = time.time()
    ...
    return {
        ...
        "mal_pct":         mal_pct,
        "features":        feats,
        "processing_time": round((time.time() - start) * 1000, 2),  # FIX [2]: unified key
    }


# =============================================================================
# SECTION 6 — STREAMLIT UI
# FIX [5]: st.set_page_config() is UNCONDITIONALLY the first Streamlit call
# =============================================================================
st.set_page_config(
    page_title="ThreatScan Pro — Professional URL Shield",
    page_icon="🛡️",
    layout="wide",
)
init_database()

# --- CSS (Gemini frontend theme) ---
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;500;600;700&family=JetBrains+Mono&display=swap');
.stApp { background-color: #0b1120 !important; color: #f1f5f9 !important; font-family: 'Space Grotesk', sans-serif; }
...
.ts-nav { display:flex; justify-content:space-between; align-items:center; padding:1rem 2rem;
          background:rgba(15,23,42,0.8); border-bottom:1px solid #1e293b; margin-bottom:2rem; }
...
.hero-tag  { background:#1e293b; border:1px solid #334155; padding:2px 10px; border-radius:12px;
              font-size:10px; font-weight:700; color:#94a3b8; text-transform:uppercase; }
.stTabs [data-baseweb="tab-list"] { gap:8px; background:rgba(15,23,42,0.5); padding:4px;
                                     border-radius:12px; border:1px solid #1e293b; width:fit-content; }
.stTabs [data-baseweb="tab"] { background:transparent !important; border-radius:8px !important;
                                color:#64748b !important; padding:8px 16px !important; font-weight:600 !important; }
.stTabs [aria-selected="true"] { background:rgba(79,70,229,0.1) !important; color:#818cf8 !important;
                                   border:1px solid rgba(79,70,229,0.2) !important; }
.stTabs [data-baseweb="tab-highlight"] { display:none !important; }
[data-testid="stForm"] { border: none !important; padding: 0 !important; }
.stTextInput > div > div > input { background-color:#0f172a !important; border:1px solid #1e293b !important;
    color:#e2e8f0 !important; border-radius:12px !important; padding:1rem !important;
...
.stButton > button, [data-testid="stFormSubmitButton"] > button {
    ...
    border-radius:12px !important; padding:0.75rem 2rem !important; font-weight:700 !important;
    width:100%; transition:0.2s; }
.stButton > button:hover, [data-testid="stFormSubmitButton"] > button:hover {
    background-color:#4338ca !important; box-shadow:0 4px 14px rgba(79,70,229,0.39) !important; }
.risk-badge { padding:4px 8px; border-radius:6px; font-size:10px; font-weight:700;
...
# --- Cache model load (runs once per server session) ---
load_model_cached = st.cache_resource(show_spinner="🛡️ Loading ThreatScan Engine...")(load_model)
model, feat_cols, _train_results = load_model_cached()

# Persistent scan result across reruns
if "last_result" not in st.session_state: st.session_state.last_result = None
if "last_scan_id" not in st.session_state: st.session_state.last_scan_id = None

# --- Nav bar ---
st.markdown("""
<div class="ts-nav">
    <div class="ts-brand"><span class="ts-icon">🛡️</span> ThreatScan Pro</div>
    <div style="color:#64748b; font-size:14px; font-weight:500;">Professional URL Shield · v5.5</div>
</div>
""", unsafe_allow_html=True)
...
<div class="ts-card" style="display:flex; justify-content:space-between; align-items:center;">
    <div style="display:flex; gap:20px; align-items:center;">
        ...
        <div>
            <h1 style="margin:0; font-size:2.5rem; font-weight:700;">ThreatScan Pro</h1>
            <div class="hero-tags">
                ...
                <span class="hero-tag">5,000-URL Dataset</span>
            </div>
        </div>
    </div>
    <div style="text-align:right;">
        <span class="hero-tag" style="margin-right:8px;">2,500 Benign</span>
        <span class="hero-tag">2,500 Malicious</span>
    </div>
</div>
""", unsafe_allow_html=True)
...
with tab_scan:
    ...
    if analyze_btn and url_input:
        with st.spinner("Analysing..."):
            ...
            try:
                ...
                c_tmp.execute(
                    'INSERT INTO scans (url, verdict, risk_score, safe_pct, mal_pct, processing_time) VALUES (?,?,?,?,?,?)',
                    (result['url'], result['verdict'], result['risk_score'],
                     result['safe_pct'], result['mal_pct'], result['processing_time']),
                )
                scan_id = c_tmp.lastrowid
                for fname, fval in result.get('features', {}).items():
                    c_tmp.execute(
                        'INSERT INTO scan_features (scan_id, feature_name, feature_value) VALUES (?,?,?)',
                        (scan_id, fname, str(fval)),
                    )
    ...
    # --- Persistent result card (session state — survives reruns) ---
    if st.session_state.last_result:
        ...
        with res_col1:
            parsed_host = html.escape(get_hostname(result['url']))
            html_card = (
                f'<div class="ts-card {border_class}">'
                f'<div style="display:flex; justify-content:space-between; align-items:start; margin-bottom:24px;">'
                f'<div style="display:flex; gap:16px; align-items:center;">'
                f'<div style="font-size:32px;">{"🚨" if is_mal else "✅"}</div>'
                f'<div><h3 style="margin:0; font-size:2rem; font-weight:700;">{result["verdict"]} RISK</h3>'
                f'<p style="margin:0; font-size:14px; opacity:0.8;">Confidence Score: {result["risk_score"]}/100</p></div></div>'
                f'<div style="text-align:right;"><div class="uppercase-label" style="opacity:0.6;">Target Host</div>'
                f'<div class="mono" style="font-size:14px;">{parsed_host}</div></div></div></div>'
            )
            ...
            if feats.get('has_punycode'):         indicators.append("Punycode / homograph domain detected.")
            ...
            # Ground-truth correction widget (v5.5 feature)
            with st.expander("📝 Report Ground Truth Correction"):
                ...
                if st.button("Apply Correction") and st.session_state.last_scan_id:
                    update_actual_label(st.session_state.last_scan_id, 0 if "Safe" in f_label else 1)
                    st.success("Correction saved to forensic log.")
        ...
        with res_col2:
            ...
            impact_data = {
                ...
                "Punycode":         70 if feats.get('has_punycode') else 2,
            }
            chart_items = [(k, v) for k, v in impact_data.items() if v > 5]
            if not chart_items:
                chart_items = [("Baseline Safe", 10)]
            chart_items.sort(key=lambda item: item[1])
            chart_labels = [i[0] for i in chart_items]
            chart_values = [i[1] for i in chart_items]
    ...
    if not recent_scans:
        st.markdown(
            ...
            'No scan history yet. Start by analysing a URL.</div>',
            unsafe_allow_html=True,
        )
    else:
        for scan in recent_scans:
            ...
            raw_url     = scan['url']
            truncated   = raw_url[:80] + '...' if len(raw_url) > 80 else raw_url
            display_url = html.escape(truncated)
            # Forensic correction indicator
            audit_tag   = ""
            if scan.get('actual_label') == 1:
                audit_tag = '<span style="color:#f87171; font-size:11px;"> ⚠️ CORRECTED: PHISH</span>'
            elif scan.get('actual_label') == 0:
                audit_tag = '<span style="color:#34d399; font-size:11px;"> ✅ CORRECTED: SAFE</span>'

            html_history = (
                f'<div style="background:rgba(30,41,59,0.3); border:1px solid #1e293b; border-radius:12px; '
                f'padding:1rem 1.5rem; margin-bottom:8px; display:flex; justify-content:space-between; align-items:center;">'
                f'<div style="display:flex; gap:16px; align-items:center;"><span class="risk-badge {badge_cls}">{scan["verdict"]}</span>'
                f'<span class="mono" style="font-size:14px; color:#cbd5e1;">{display_url}</span>{audit_tag}</div>'
                f'<span style="opacity:0.6; font-size:12px;">{scan["timestamp"][:16]}</span></div>'
            )
...
with tab_perf:
    ...
    st.caption(
        "All three classifiers are trained on the same 80/20 stratified split so results are directly "
        "comparable. The best-accuracy model is used for live scans. "
        "Dataset: 2,500 benign + 2,500 malicious (5,000 total)."
    )
    ...
    if not comparison:
        st.info("Performance metrics appear here after the first scan triggers training. "
        ...
        for name, d in comparison.items():
            rows.append({
                ...
                "FRR":       f"{'🟢' if d['frr'] <= 0.03 else '🔴'} {d['frr']*100:.2f}%",
                "AUC":       f"{d['auc']:.3f}",
            })
        st.dataframe(pd.DataFrame(rows), use_container_width=True)
        st.info("🎯 Performance Targets: Accuracy ≥ 95% | FAR ≤ 2% (🟢) | FRR ≤ 3% (🟢)")

        # Only list models present in both sources — guards against stale joblib drift
        model_names = [n for n in _train_results.keys() if n in comparison]
        if model_names:
            ...
            # Guard: stale joblib from another codebase version may lack plot keys
            if not all(k in sel for k in ("y_test", "y_pred", "y_prob")):
                st.warning("⚠️ Chart data missing. Delete `models/best_model.joblib` and restart to retrain.")
            else:
                ...
                with col_cm:
                    ...
                    # labels=[0,1] forces a 2x2 matrix even if one class is absent
                    ConfusionMatrixDisplay(
                        confusion_matrix(y_test_s, y_pred_s, labels=[0, 1]),
                ...
                with col_roc:
                    ...
                    # Guard BEFORE roc_curve: it raises ValueError if only one class is present
                    if len(np.unique(y_test_s)) > 1:
                        fpr, tpr, _ = roc_curve(y_test_s, y_prob_s)
                        auc_val = roc_auc_score(y_test_s, y_prob_s)
                    else:
                    ...
                    ax_roc.spines['bottom'].set_color('#334155'); ax_roc.spines['left'].set_color('#334155')
        ...
        ax_ff.spines['bottom'].set_color('#334155'); ax_ff.spines['left'].set_color('#334155')
        ax_ff.spines['top'].set_visible(False); ax_ff.spines['right'].set_visible(False)
        plt.tight_layout()
        st.pyplot(fig_ff, use_container_width=True)
        plt.close(fig_ff)

if __name__ == "__main__":
    # Guard: streamlit.runtime API varies across versions
    _inside_streamlit = False
    try:
        import streamlit.runtime as _st_runtime
        _inside_streamlit = _st_runtime.exists()
    except Exception:
        pass
